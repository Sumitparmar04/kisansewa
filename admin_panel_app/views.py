...
from web_panel_app.models import *
...
def add_product(request):
    if request.method == 'GET':
        sub_cat = Sub_Category.objects.all().exclude(id=9)
        company = Company.objects.all().exclude(id=3)
...
def update_product(request):
    if request.method == 'GET':
        ...
        sub_cat = Sub_Category.objects.all().exclude(id=5)
        company = Company.objects.all().exclude(id=3)
        media1 = media
...
def add_sub_cat(request):
    if request.method == 'POST':
        ...
        category = Category.objects.all().exclude(id=7)
...
def cart_list(request):
    user_id = request.GET.get('user_id')
    cart = Cart.objects.filter(customer = user_id).order_by('-updated_at')
